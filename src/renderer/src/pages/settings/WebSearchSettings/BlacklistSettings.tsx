...
const BlacklistSettings: FC = () => {
  ...
  async function updateSubscribe() {
    ...
    try {
      // Get selected subscription sources
      const selectedSources = dataSource.filter((item) => selectedRowKeys.includes(item.key))

      // Store all successfully parsed subscription data
      ...
      for (const source of selectedSources) {
        try {
          ...
          // Show specific update failure message
  ...
  // Modified handleAddSubscribe function
  ...
  function handleDeleteSubscribe() {
    try {
      // Filter out selected items to be deleted
